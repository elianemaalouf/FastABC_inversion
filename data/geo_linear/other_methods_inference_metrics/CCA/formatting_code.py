...
cca_metrics_file = "FastABC/data/geo_linear/other_methods_inference_metrics/CCA/inversion_metrics.pkl"
cca_data_file = "FastABC/data/geo_linear/other_methods_inference_metrics/CCA/inversion_data.pkl"
...
save_to_disk(cca_data_formatted,
             "FastABC/data/geo_linear/other_methods_inference_metrics/CCA/inversion_data_formatted.pkl")
...
for metric in metrics.keys():
    ...
    for noise_label in noise_list:
        if noise_label == 'small_noise':
            noise_label = 'small_gauss'
        elif noise_label == 'large_noise':
            noise_label = 'large_gauss'

        if metric == 'rmse':
            cca_metrics_formatted[metric][noise_label] = cca_metrics[noise_label][metric]
            ...
            for p in metrics[metric]:
                ...
                if p == 1:
                    cca_metrics_formatted[metric][p][noise_label] = cca_metrics[noise_label]['es']
                elif p == 2:
                    es_stats = []
                    ...
                    cca_metrics_formatted[metric][p][noise_label] = es_stats
                else:
                    pass
            ...
            for p in metrics[metric]:
                if p not in cca_metrics_formatted[metric].keys():
                    cca_metrics_formatted[metric][p] = {}
                cca_metrics_formatted[metric][p][noise_label] = cca_metrics[noise_label]['vs']
        else:
            pass

save_to_disk(cca_metrics_formatted,
             "FastABC/data/geo_linear/other_methods_inference_metrics/CCA/inversion_metrics_formatted.pkl")
