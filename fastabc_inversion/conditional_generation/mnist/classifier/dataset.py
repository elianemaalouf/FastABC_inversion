...
def get(batch_size, data_root='/tmp/public_dataset/pytorch', img_size = 32, train=True, val=True, **kwargs):
        ...
        test_loader = torch.utils.data.DataLoader(
            datasets.MNIST(root=data_root, train=False, download=True,
                           transform=transforms.Compose([
                                transforms.Resize(img_size),
                                transforms.ToTensor(),
                                transforms.Normalize(mean=(0.5,), std=(0.5,)) # modified normalization by Eliane Maalouf
