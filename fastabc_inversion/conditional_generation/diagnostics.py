...
class Diagnostics:
    ...
    def load_classifier(self, classifier, append_softmax=False):
        ...
        if not append_softmax:
            ...
            self.classifier = torch.nn.Sequential(classifier, torch.nn.Softmax(dim=1))
    ...
    def build_diagnostics_data(self, sample_size=None):
        ...
        self.sample_size = (
            self.experiment.val_size if sample_size is None else sample_size
        )
        ...
        train_random_indices = torch.randperm(self.experiment.train_size)[
            : self.sample_size
        ]
        ...
        self.train_y_label = self.experiment.label_transform.simplex_vec_to_label(
            self.train_y
        )

        val_random_indices = torch.randperm(self.experiment.val_size)[
            : self.sample_size
        ]
        ...
        all_val_samples = [
            self.validation_data.dataset[i]
            for i in range(len(self.validation_data.dataset))
        ]
        ...
        self.val_y_label = self.experiment.label_transform.simplex_vec_to_label(
            self.val_y
        )
        ...
        with torch.no_grad():
            ...
            for batch in self.training_data:
                ...
                if self.classifier is not None:
                    classified_recon_x = self.classifier(
                        reconstructed_train[0].to(device)
                    )
                    resimulated_recon_train_x_list.append(classified_recon_x.cpu())
                else:
                    # warning message if no classifier is loaded
                    print(
                        "Warning: No classifier loaded. resimulated_recon_train_x will not be computed."
                    )
            ...
            self.reconstructed_train_y_label = (
                self.experiment.label_transform.simplex_vec_to_label(
                    self.reconstructed_train_y
                )
            )

            if self.classifier is not None:
                self.resimulated_recon_train_x = torch.cat(
                    resimulated_recon_train_x_list, dim=0
                )
                self.resimulated_recon_train_x_label = (
                    self.experiment.label_transform.simplex_vec_to_label(
                        self.resimulated_recon_train_x
                    )
                )
            ...
            for batch in self.validation_data:
                ...
                if self.classifier is not None:
                    classified_recon_x = self.classifier(
                        reconstructed_val[0].to(device)
                    )
                    resimulated_recon_val_x_list.append(classified_recon_x.cpu())
                else:
                    # warning message if no classifier is loaded
                    print(
                        "Warning: No classifier loaded. resimulated_recon_val_x_list will not be computed."
                    )
            ...
            self.reconstructed_val_y_label = (
                self.experiment.label_transform.simplex_vec_to_label(
                    self.reconstructed_val_y
                )
            )
            if self.classifier is not None:
                self.resimulated_recon_val_x = torch.cat(
                    resimulated_recon_val_x_list, dim=0
                )
                self.resimulated_recon_val_x_label = (
                    self.experiment.label_transform.simplex_vec_to_label(
                        self.resimulated_recon_val_x
                    )
                )
            ...
            self.generated_y_label = (
                self.experiment.label_transform.simplex_vec_to_label(self.generated_y)
            )
            # classify generated_x and store in resimulated_generated_x
            if self.classifier is not None:
                self.resimulated_generated_x = self.classifier(
                    self.generated_x.to(device)
                ).cpu()
                self.resimulated_generated_x_label = (
                    self.experiment.label_transform.simplex_vec_to_label(
                        self.resimulated_generated_x
                    )
                )
            else:
                # warning message if no classifier is loaded
                print(
                    "Warning: No classifier loaded. resimulated_generated_x will not be computed."
                )
    ...
    def setup_logging(self, logging_comment=""):
        ...
        decoder_model_selector = {
            "decoder_model_selector": self.experiment.model_selector
        }
        decoder_smooth = {
            "decoder_smoothness": self.experiment.nn_params["netG_apply_spectral_norm"]
        }
        ...
        lambda_sink_scheduling = {
            "lambda_sink_scheduling": self.experiment.sinkhorn_lambda_scheduling_params[
        ...
        inflate_recon_y = {
            "inflate_recon_y": self.experiment.model_training_params["inflate_recon_y"]
        }
    ...
    def estimate_mmd_params(self, which_variable, max_sample_size=10000):
        ...
        if which_variable in ["x", "y"]:
            ...
            all_train_samples = [
                data_loader.dataset[i] for i in range(len(data_loader.dataset))
            ]
            ...
            all_data = np.vstack(
                (all_train_latent_codes, all_val_latent_codes, large_latent_vector)
            )
    ...
    def run_diagnostics(
        ...
        if "inspect_latent_distribution" in diagnostics_to_run and (
            self.mmd_est_params["z"] is None
        ):
            self.estimate_mmd_params(which_variable="z", max_sample_size=10000)
        if "prior_vs_gen_dist_distance" in diagnostics_to_run and (
            self.mmd_est_params["x"] is None
        ):
    ...
    def make_priorvsgen_dists_distances_stats_claude(
        ...
        train_x = (
            torch.cat(all_train_samples, dim=0).view(-1, self.experiment.dim_x).numpy()
        )
        del all_train_samples
    ...
    def make_priorvsgen_dists_distances_stats(
        self, mmd_params=None, sample_size=300, repeats=100, two_sample_test_params=None
    ):
        """
        ...
        two_samples_test_p_values = (
            []
        )  # store all p-value from two-sample tests repetitions
        two_samples_test_rejections = (
            []
        )  # store all rejections results from two-sample tests repetitions
        mmd_x_dists = []
        mmd_x_refs = []
        ...
        print(
            f"Using estimated MMD X kernel parameter gamma: {self.mmd_est_params['x']}"
        )
        ...
        for i in range(repeats):
            ...
            train_loader_sample = DataLoader(
                Subset(
                    self.experiment.train_loader.dataset,
            ...
            all_train_samples = [
                train_loader_sample.dataset[i]
                for i in range(len(train_loader_sample.dataset))
            ]
            all_train_samples = (
                torch.stack([sample[0] for sample in all_train_samples])
                .view(-1, self.experiment.dim_x)
                .numpy()
            )
            ...
            latent_vector = (
                ...
                .sample((sample_size, self.experiment.latent_dim))
                .to(self.experiment.device)
            )
            with torch.no_grad():
            ...
            if compute_mmd:
                ...
                mmd_x_dists.append(
                    mmd(train_sample_1, generated_sample, **mmd_params)[0]
                )
            ...
            if two_sample_test_params is not None:
                ...
                # store p-value
                test_result = two_sample_mmd_test(
                    train_sample_1, generated_sample, **two_sample_test_params
                )
        ...
        if two_samples_test_p_values is not None:
            ...
            combined_p_value = 1 - chi2.cdf(
                combined_stat, 2 * len(two_samples_test_p_values)
            )
        ...
        write_combined_fisher_pvalue_two_sample_x = (
            f"{combined_p_value:.{precision}f}"
            if combined_p_value is not None
            else None
        )
        ...
        write_header_combined_fisher_pvalue_two_sample_x = (
            "combined_pvalue_two_sample_x"
        )

        write_rejection_prop_two_sample_x = (
            f"{rejection_proportion:.{precision}f}"
            if rejection_proportion is not None
            else None
        )
        ...
        return {
            "data": {
                ...
                "write_combined_fisher_pvalue_two_sample_x": (
                    write_combined_fisher_pvalue_two_sample_x,
                    write_combined_fisher_pvalue_two_sample_x_txt,
                ),
                "write_rejection_prop_two_sample_x": (
                    write_rejection_prop_two_sample_x,
                    write_rejection_prop_two_sample_x_txt,
                ),
            },
            "headers": {
                "write_header_mmd_x": write_header_mmd_x,
    ...
    def make_priorvsgen_dists_distances_stats_2(
        self,
        mmd_params=None,
        sample_size=300,
        repeats=100,
        two_sample_test_params=None,
        compute_only_with_rand_proj=True,
        rand_proj_dims=30,
    ):
        # TODO : delete
        """
        ...
        """
        print(
            "Making prior vs generated distributions distance statistics for IMAGE data only..."
        )
        precision = self.round_digits
        ...
        if compute_mmd or two_sample_test_params is not None:
            ...
            proj_train_x = random_proj_x.fit_transform(
                train_x.reshape(-1, self.experiment.dim_x)
            )
        ...
        for i in range(repeats):
            ...
            train_x_sample = (
                train_x[idx_rnd, :, :, :].reshape(-1, self.experiment.dim_x)
                if len(train_x.shape) > 2
                else train_x[idx_rnd, :].reshape(-1, self.experiment.dim_x)
            )

            train_x_sample_2 = (
                train_x[idx_rnd_2, :, :, :].reshape(-1, self.experiment.dim_x)
                if len(train_x.shape) > 2
                else train_x[idx_rnd_2, :].reshape(-1, self.experiment.dim_x)
            )
            ...
            if compute_mmd:
                ...
                if not compute_only_with_rand_proj:
                    # compute reference
                    mmd_x_refs.append(
                        mmd(train_x_sample, train_x_sample_2, **mmd_params)[0]
                    )
                    mmd_x_dists.append(
                        mmd(train_x_sample, generated_x_sample, **mmd_params)[0]
                    )
                else:
                    mmd_x_refs = mmd_x_dists = None

                mmd_proj_x_refs.append(
                    mmd(proj_train_x_sample, proj_train_x_sample_2, **mmd_params)[0]
                )
                mmd_proj_x_dists.append(
                    mmd(proj_train_x_sample, proj_gen_x_sample, **mmd_params)[0]
                )
            ...
            if two_sample_test_params is not None:
                ...
                if not compute_only_with_rand_proj:
                    ...
                    two_sample_tests_x.append(
                        two_sample_mmd_test(
                            train_x_sample, generated_x_sample, **two_sample_test_params
                        )[2]
                    )
                else:
                ...
                two_sample_tests_proj_x.append(
                    two_sample_mmd_test(
                        proj_train_x_sample, proj_gen_x_sample, **two_sample_test_params
                    )[2]
                )
        ...
        mmd_x_dists = np.array(mmd_x_dists) if mmd_x_dists is not None else None
        mmd_proj_x_dists = (
            np.array(mmd_proj_x_dists) if mmd_proj_x_dists is not None else None
        )

        mmd_x_refs = np.array(mmd_x_refs) if mmd_x_refs is not None else None
        mmd_proj_x_refs = (
            np.array(mmd_proj_x_refs) if mmd_proj_x_refs is not None else None
        )

        two_sample_tests_x = (
            np.array(two_sample_tests_x) if two_sample_tests_x is not None else None
        )
        two_sample_tests_proj_x = (
            np.array(two_sample_tests_proj_x)
            if two_sample_tests_proj_x is not None
            else None
        )
        ...
        if two_sample_test_params is not None:
            prop_reject_two_sample_x = (
                np.mean(two_sample_tests_x) if two_sample_tests_x is not None else None
            )
            prop_reject_two_sample_proj_x = np.mean(two_sample_tests_proj_x)
        else:
            prop_reject_two_sample_x = prop_reject_two_sample_proj_x = None

        write_prop_reject_two_sample_x = (
            f"{prop_reject_two_sample_x:.{precision}f}"
            if prop_reject_two_sample_x is not None
            else None
        )
        ...
        write_prop_reject_two_sample_proj_x = (
            f"{prop_reject_two_sample_proj_x:.{precision}f}"
            if prop_reject_two_sample_proj_x is not None
            else None
        )
        ...
        write_data_mmd_proj_x = self.make_array_stats_string(
            mmd_proj_x_dists_stats, precision
        )
        ...
        write_data_mmd_proj_x_ref = self.make_array_stats_string(
            mmd_proj_x_refs_stats, precision
        )
        ...
        return {
            "data": {
                "write_data_mmd_x": (write_data_mmd_x, write_data_mmd_x_txt),
                "write_data_mmd_proj_x": (
                    write_data_mmd_proj_x,
                    write_data_mmd_proj_x_txt,
                ),
                "write_prop_reject_two_sample_x": (
                    write_prop_reject_two_sample_x,
                    write_prop_reject_two_sample_x_txt,
                ),
                "write_prop_reject_two_sample_proj_x": (
                    write_prop_reject_two_sample_proj_x,
                    write_prop_reject_two_sample_proj_x_txt,
                ),
                "write_data_mmd_x_ref": (
                    write_data_mmd_x_ref,
                    write_data_mmd_x_ref_txt,
                ),
                "write_data_mmd_proj_x_ref": (
                    write_data_mmd_proj_x_ref,
                    write_data_mmd_proj_x_ref_txt,
                ),
            },
            "headers": {
                "write_header_mmd_x": write_header_mmd_x,
    ...
    def make_recons_stats(self, build_y_stats=True, make_plots=True):
        ...
        if self.train_x is not None and self.train_y is not None:
            recon_train_x_rmse_vec = rmse_torch(
                self.reconstructed_train_x.view(-1, dim_x), self.train_x.view(-1, dim_x)
            ).numpy()
            ...
            if build_y_stats:
                ...
                recon_y_label_accuracy = compare_labels(
                    self.train_y_label, self.reconstructed_train_y_label
                )
            else:
            ...
            write_data_recon_train_y_txt = (
                "Reconstructed training label vs training label accuracy: "
            )
        ...
        if self.val_x is not None and self.val_y is not None:
            recon_val_x_rmse_vec = rmse_torch(
                self.reconstructed_val_x.view(-1, dim_x), self.val_x.view(-1, dim_x)
            ).numpy()
            ...
            if build_y_stats:
                ...
                recon_val_y_label_accuracy = compare_labels(
                    self.val_y_label, self.reconstructed_val_y_label
                )
            else:
            ...
            write_data_recon_val_y = (
                f'{self.results_vecs["recon_val_y_label_accuracy"]}'
            )
            write_data_recon_val_y_txt = (
                "Reconstructed validation label vs validation label accuracy:"
            )
    ...
    def plot_true_vs_recon(self, data_type="train", grid_size=4, dpi=600, show=False):
        ...
        total_sample_size = (
            self.train_x.shape[0] if data_type == "train" else self.val_x.shape[0]
        )
        ...
        dim_x = self.experiment.dim_x
        rmse_x = rmse_torch(
            recon_x_data.view(-1, dim_x), x_data.view(-1, dim_x)
        ).numpy()
    ...
    def assess_forward_learning(self, make_plots=True, dpi=600, show=False):
        ...
        if self.train_x is not None and self.train_y is not None:
            resimulated_recon_train_x_label_accuracy = compare_labels(
                self.resimulated_recon_train_x_label, self.reconstructed_train_y_label
            )
            self.results_vecs[
                "resimulated_recon_train_x_label_accuracy"
            ] = resimulated_recon_train_x_label_accuracy

            write_data_resimulated_recon_train_x = (
                f'{self.results_vecs["resimulated_recon_train_x_label_accuracy"]}'
            )
            write_data_resimulated_recon_train_x_txt = "(Train) Resimulated reconstructed image labels vs reconstructed label accuracy: "
            write_header_resimulated_recon_train_x = (
                "resimulated_recon_train_x_label_accuracy"
            )

        else:
            raise ValueError(
                "No training data to compute forward model learning assessment."
                "Load data into the experiment object first."
            )

        if self.val_x is not None and self.val_y is not None:
            resimulated_recon_val_x_label_accuracy = compare_labels(
                self.resimulated_recon_val_x_label, self.reconstructed_val_y_label
            )
            self.results_vecs[
                "resimulated_recon_val_x_label_accuracy"
            ] = resimulated_recon_val_x_label_accuracy

            write_data_resimulated_recon_val_x = (
                f'{self.results_vecs["resimulated_recon_val_x_label_accuracy"]}'
            )
            write_data_resimulated_recon_val_x_txt = "(Val) Resimulated reconstructed image labels vs reconstructed label accuracy: "
            write_header_resimulated_recon_val_x = (
                "resimulated_recon_val_x_label_accuracy"
            )

        else:
            raise ValueError(
                "No validation data to compute forward model learning assessment."
                "Load data into the experiment object first."
            )

        # compare resimulated_generated_x_label to train_y_label
        resimulated_generated_x_label_accuracy = compare_labels(
            self.resimulated_generated_x_label, self.generated_y_label
        )
        self.results_vecs[
            "resimulated_generated_x_label_accuracy"
        ] = resimulated_generated_x_label_accuracy

        write_data_resimulated_generated_x = (
            f'{self.results_vecs["resimulated_generated_x_label_accuracy"]}'
        )
        write_data_resimulated_generated_x_txt = (
            "Resimulated generated image vs generated label accuracy: "
        )
        ...
        gen_y_data_label = self.generated_y_label[random_indices_gen].unsqueeze(
            1
        )  # generated data labels
        resim_gen_x_data_label = self.resimulated_generated_x_label[
            random_indices_gen
        ].unsqueeze(
            1
        )  # resimulated generated data labels

        train_x_data = self.train_x[random_indices_train].squeeze()  # true train data
        recon_train_x_data = self.reconstructed_train_x[
            random_indices_train
        ].squeeze()  # reconstructed train data
        true_train_y_label = self.train_y_label[random_indices_train].unsqueeze(
            1
        )  # true train data labels
        resim_recon_train_x_data_label = self.resimulated_recon_train_x_label[
            random_indices_train
        ].unsqueeze(
            1
        )  # resimulated reconstructed train data labels
        rmse_train_x = rmse_torch(
            recon_train_x_data.view(-1, dim_x), train_x_data.view(-1, dim_x)
        ).numpy()

        val_x_data = self.val_x[random_indices_val].squeeze()  # true val data
        recon_val_x_data = self.reconstructed_val_x[
            random_indices_val
        ].squeeze()  # reconstructed val data
        true_val_y_label = self.val_y_label[random_indices_val].unsqueeze(
            1
        )  # true val data labels
        resim_recon_val_x_data_label = self.resimulated_recon_val_x_label[
            random_indices_val
        ].unsqueeze(
            1
        )  # resimulated reconstructed val data labels
        rmse_val_x = rmse_torch(
            recon_val_x_data.view(-1, dim_x), val_x_data.view(-1, dim_x)
        ).numpy()
        ...
        train_plot_y_data = torch.cat(
            [true_train_y_label, resim_recon_train_x_data_label], dim=0
        ).numpy()
        val_plot_y_data = torch.cat(
            [true_val_y_label, resim_recon_val_x_data_label], dim=0
        ).numpy()
        gen_plot_y_data = torch.cat(
            [gen_y_data_label, resim_gen_x_data_label], dim=1
        ).numpy()
        ...
        return {
            "data": {
                "write_data_resimulated_recon_train_x": (
                    write_data_resimulated_recon_train_x,
                    write_data_resimulated_recon_train_x_txt,
                ),
                "write_data_resimulated_recon_val_x": (
                    write_data_resimulated_recon_val_x,
                    write_data_resimulated_recon_val_x_txt,
                ),
                "write_data_resimulated_generated_x": (
                    write_data_resimulated_generated_x,
                    write_data_resimulated_generated_x_txt,
    ...
    def inspect_latent_distribution_2(
        self,
        make_plots=True,
        n_neighbors=100,
        min_dist=0.3,
        ...
        if two_sample_test_params is not None:
            ...
            if self.mmd_est_params["z"] is None:
                self.estimate_mmd_params(which_variable="z", max_sample_size=10000)
            self.two_sample_test_params["kernel_params"]["gamma"] = self.mmd_est_params[
                "z"
            ]
            self.mmd_params["kernel_params"]["gamma"] = self.mmd_est_params["z"]
            print(
                f"Using estimated MMD Z kernel parameter gamma: {self.mmd_est_params['z']}"
            )
            ...
            for i in range(test_repeats):
                ...
                idx_train = np.random.choice(
                    self.latent_train_codes.shape[0], sample_size, replace=False
                )
                latent_train_codes = self.latent_train_codes[idx_train, :].numpy()
                idx_val = np.random.choice(
                    self.latent_val_codes.shape[0], sample_size, replace=False
                )
                latent_val_codes = self.latent_val_codes[idx_val, :].numpy()

                train_test = two_sample_mmd_test(
                    latent_vector, latent_train_codes, **two_sample_test_params
                )
                val_test = two_sample_mmd_test(
                    latent_vector, latent_val_codes, **two_sample_test_params
                )
                ...
                if mmd_params is None:
                    ...
                    mmd_params = {
                        "kernel_params": self.two_sample_test_params[
                            "kernel_params"
                        ].copy(),
                        "unbiased": False,
                    }
                    ...
                    latent_train_mmd_values_vec.append(
                        mmd(latent_vector, latent_train_codes, **mmd_params)[0]
                    )
                    latent_val_mmd_values_vec.append(
                        mmd(latent_vector, latent_val_codes, **mmd_params)[0]
                    )

                latent_mmd_ref.append(
                    mmd(latent_vector, latent_vector_2, **mmd_params)[0]
                )
        ...
        if latent_train_mmd_two_sample_tests_pvalues is not None:
            ...
            rejection_prop_train = np.mean(
                np.array(latent_train_mmd_two_sample_tests_rejections)
            )
            ...
            rejection_prop_val = np.mean(
                np.array(latent_val_mmd_two_sample_tests_rejections)
            )
        else:
            combined_p_value_train = (
                combined_p_value_val
            ) = rejection_prop_train = rejection_prop_val = None
        ...
        write_header_combined_pvalue_train_mmd_two_sample_test = (
            "combined_pvalue_train_mmd_two_sample_test"
        )

        write_rejection_prop_train_mmd_two_sample_test = (
            f"{rejection_prop_train:.{precision}f}"
            if rejection_prop_train is not None
            else None
        )
        ...
        write_header_rejection_prop_train_mmd_two_sample_test = (
            "rejection_prop_train_mmd_two_sample_test"
        )
        ...
        write_header_combined_pvalue_val_mmd_two_sample_test = (
            "combined_pvalue_val_mmd_two_sample_test"
        )

        write_rejection_prop_val_mmd_two_sample_test = (
            f"{rejection_prop_val:.{precision}f}"
            if rejection_prop_val is not None
            else None
        )
        ...
        write_header_rejection_prop_val_mmd_two_sample_test = (
            "rejection_prop_val_mmd_two_sample_test"
        )
        ...
        write_header_train_mmd_values = (
            "mean_latent_train_mmd_values,median_latent_train_mmd_values,"
            "[(q25_latent_train_mmd_values,q75_latent_train_mmd_values),"
            "(q025_latent_train_mmd_values,q975_latent_train_mmd_values)]"
        )
        ...
        write_data_val_mmd_values_txt = (
            f"MMD values - validation latent codes {test_sample_size * test_repeats}: "
        )
        write_header_val_mmd_values = (
            "mean_latent_val_mmd_values,median_latent_val_mmd_values,"
            "[(q25_latent_val_mmd_values,q75_latent_val_mmd_values),"
            "(q025_latent_val_mmd_values,q975_latent_val_mmd_values)]"
        )
        ...
        write_header_mmd_ref_values = (
            "mean_mmd_ref_values,median_mmd_ref_values,"
            "[(q25_mmd_ref_values,q75_mmd_ref_values),"
            "(q025_mmd_ref_values,q975_mmd_ref_values)]"
        )
        ...
        return {
            "data": {
                ...
                "write_rejection_prop_train_mmd_two_sample_test": (
                    write_rejection_prop_train_mmd_two_sample_test,
                    write_rejection_prop_train_mmd_two_sample_test_txt,
                ),
                "write_data_train_mmd_values": (
                    write_data_train_mmd_values,
                    write_data_train_mmd_values_txt,
                ...
                "write_rejection_prop_val_mmd_two_sample_test": (
                    write_rejection_prop_val_mmd_two_sample_test,
                    write_rejection_prop_val_mmd_two_sample_test_txt,
                ),
                "write_data_val_mmd_values": (
                    write_data_val_mmd_values,
                    write_data_val_mmd_values_txt,
    ...
    def plot_latent_umap_tsne_scatters(
        ...
        **kwargs,
        ...
        if fit_prior_only and denseMAP:
            if type == "umap":
                print(
                    "Warning: denseMAP cannot be used when fit_prior_only is True. Setting denseMAP to False."
                )
                denseMAP = False
        ...
        def make_dim_redux(type):
            ...
            extra_string_comment = (
                f"_fit_prior_only{fit_prior_only}_denseMAP{denseMAP}"
                if type == "umap"
                else f"_fit_prior_only{fit_prior_only}"
            )
            plot.plot_scatters(
                data=embedding_data,
                labels=labels,
        ...
        def make_dim_redux_2(type):
            ...
            if not fit_prior_only:
                ...
                embeddings_latent_train = embeddings[
                    latent_train_id_start:latent_train_id_end
                ]
                embeddings_latent_val = embeddings[
                    latent_val_id_start:latent_val_id_end
                ]
            else:
                embeddings_latent = embeddings
                embeddings_latent_train = reducer.transform(
                    self.latent_train_codes.numpy()
                )
            ...
            for digit in range(10):
                ...
                if np.any(mask):
                    ax.scatter(
                        embeddings_latent_train[mask, 0],
                        embeddings_latent_train[mask, 1],
                        c=[digit_colors[digit]],
                        marker=markers["Training"],
                        s=marker_sizes["Training"],
                        alpha=0.7,
                        label=f"Train-{digit}",
                        edgecolors="white",
                        linewidths=0.5,
            ...
            for digit in range(10):
                ...
                if np.any(mask):
                    ax.scatter(
                        embeddings_latent_val[mask, 0],
                        embeddings_latent_val[mask, 1],
                        c=[digit_colors[digit]],
                        marker=markers["Validation"],
                        s=marker_sizes["Validation"],
                        alpha=0.7,
                        label=f"Val-{digit}",
                        edgecolors="white",
                        linewidths=0.5,
            ...
            ax.set_title(
                f"{type.upper()} embedding of latent codes{optional_string}",
                fontsize=14,
            )
            ...
            if show:
                ...
                plt.savefig(
                    f"{self.latent_space_diag_dir}/latent_{type}{extra_string_comment}_scatter.pdf",
                    dpi=dpi,
                    bbox_inches="tight",
                )
            plt.close()
    ...
    def plot_x_y_umap_tsne_scatters_2(
        self,
        n_neighbors=100,
        min_dist=0.3,
        dpi=600,
        ...
        **kwargs,
        ...
        def make_dim_redux(data, labels_list, title_string, data_types, type):
            ...
            for data_idx, (emb, labels, data_type) in enumerate(
                zip(embeddings_list, labels_list, data_type_names)
            ):
                for digit in range(10):
            ...
            ax.set_title(
                f"{type.upper()} embedding of {title_string}{optional_string}",
                fontsize=14,
            )
            ax.set_xlabel(f"{type.upper()} 1", fontsize=12)
            ax.set_ylabel(f"{type.upper()} 2", fontsize=12)
            ...
            if show:
                ...
                plt.savefig(
                    f"{self.logging_dir}/{type}_scatter_{title_string}{extra_string_comment}.pdf",
                    dpi=dpi,
                    bbox_inches="tight",
                )
            plt.close()
        ...
        if (
            ...
            raise ValueError(
                "No generated or reconstructed data to plot. Build diagnostics data first."
            )
        ...
        make_dim_redux(
            ...
            data_types=["Train", "Generated"],
            title_string="train_vs_generated",
            type="umap",
        )
        make_dim_redux(
            [train_x, generated_x],
        ...
        make_dim_redux(
            [train_x, recon_val_x],
            [train_labels, recon_val_labels],
            data_types=["Train", "Recon.-Train"],
            title_string="train_vs_recons_train",
            type="umap",
        )
        make_dim_redux(
            [train_x, recon_val_x],
        ...
        make_dim_redux(
            [train_x, recon_val_x],
            [train_labels, recon_val_labels],
            data_types=["Train", "Recon.-Val"],
            title_string="train_vs_recons_val",
            type="umap",
        )
        make_dim_redux(
            [train_x, recon_val_x],
    ...
    def assess_class_proportions(self, **kwargs):
        ...
        with torch.no_grad():
            ...
            gen_labels = self.experiment.label_transform.simplex_vec_to_label(
                generated_y
            )

            classified_generated_x = self.classifier(generated_x)
            classified_labels = self.experiment.label_transform.simplex_vec_to_label(
                classified_generated_x
            )

            plot_samples_inspections(
                generated_x.cpu(),
                gen_labels,
                labels_2=classified_labels,
                random_samples=False,
                grid_size=10,
                save_fig_path=f"{self.logging_dir}/generated_samples_by_true_labels.pdf",
                dpi=600,
            )
            ...
            for c in range(self.experiment.dim_y):
                ...
                classified_gen_class_counts[c] += torch.sum(
                    classified_labels == c
                ).item()

        gen_class_proportions = gen_class_counts / np.sum(gen_class_counts)
        classified_gen_class_proportions = classified_gen_class_counts / np.sum(
            classified_gen_class_counts
        )

        # perform Chi-squared test
        chi2_stat, p_value = chisquare(gen_class_counts, f_exp=train_class_counts)
        chi2_stat_classified, p_value_classified = chisquare(
            classified_gen_class_counts, f_exp=train_class_counts
        )
        ...
        print(
            f"Classified Generated data class proportions: {classified_gen_class_proportions}"
        )
        print(f"Chi-squared statistic: {chi2_stat:.4f}, p-value: {p_value:.4f}")
        print(
            f"Classified Chi-squared statistic: {chi2_stat_classified:.4f}, p-value: {p_value_classified:.4f}"
        )
        ...
        bars3 = ax.bar(
            x + offsets[2],
            classified_gen_class_proportions,
            width,
            label="Classified Generated Data",
    ...
    def assess_frechet_scores(self, pca_components=0.85, sample_size=300, repeats=100):
        """
        ...
        all_train_dataset = DataLoader(
            all_train_dataset, batch_size=self.experiment.train_size, shuffle=False
        )
        ...
        pca.fit(all_train_x)
        print(
            f"PCA fitted on the full training data with {pca.n_components_} components."
        )
        del all_train_x
        ...
        for r in range(repeats):
            ...
            train_indices = np.random.choice(
                train_size, size=sample_size, replace=False
            )
            train_indices_2 = np.random.choice(
                train_size, size=sample_size, replace=False
            )
        ...
        write_data_frechet_scores_gen_x = self.make_array_stats_string(
            fpd_scores_stats, precision
        )
        write_data_frechet_scores_gen_x_txt = (
            "Frechet distance in PCA space between training and generated images: "
        )
        ...
        write_data_frechet_scores_gen_x_refs = self.make_array_stats_string(
            fpd_scores_refs_stats, precision
        )
        ...
        return {
            "data": {
                "write_data_frechet_scores_gen_x": (
                    write_data_frechet_scores_gen_x,
                    write_data_frechet_scores_gen_x_txt,
                ),
                "write_data_frechet_scores_gen_x_refs": (
                    write_data_frechet_scores_gen_x_refs,
                    write_data_frechet_scores_gen_x_txt_refs,
                ),
            },
            "headers": {
...
def mmd_with_testset(
    experiment,
    inference_sample,
    inverted_vec,
    makes_refs=False,
    storing_directory=None,
    filename_ref=None,
):
    """
    ...
    """
    import fastabc_inversion.conditional_generation.utils.mmd as mmd

    print(
        f"Computing MMD statistics between inference sample and test set for {inverted_vec}..."
    )
    ...
    sq_median_test = mmd.estimate_median_pairwise_dists(
        combined_data, sample_ratio=1.0, chunk_size=300
    )
    ...
    for _ in range(num_repeats):
        ...
        mmd_values_test.append(
            mmd.MMD2(
                inference_sample,
                test_subset,
                kernel_params=mmd_kernel_params_test,
                unbiased=False,
            )[0]
        )
    ...
    if storing_directory is not None:
        ...
        with open(
            f"{storing_directory}/mmd_inference_vs_testset_values_{filename_ref}.pkl",
            "wb",
        ) as f:
        ...
        with open(
            f"{storing_directory}/mmd_inference_vs_testset_stats_{filename_ref}.txt",
            "w",
        ) as f:
    ...
    if makes_refs:
        print(
            "Computing MMD statistics between training set and test set as reference..."
        )
        ...
        sq_median_ref = mmd.estimate_median_pairwise_dists(
            combined_data_ref, sample_ratio=1.0, chunk_size=300
        )
        ...
        for _ in range(num_repeats):
            if train_size <= ref_sample_size:
                ...
                indices = np.random.choice(
                    train_size, size=ref_sample_size, replace=False
                )
            ...
            if test_size <= ref_sample_size:
                ...
                indices = np.random.choice(
                    test_size, size=ref_sample_size, replace=False
                )
                test_subset = test_x[indices]

            mmd_values_ref.append(
                mmd.MMD2(
                    train_subset,
                    test_subset,
                    kernel_params=mmd_kernel_params_ref,
                    unbiased=False,
                )[0]
            )
        ...
        if storing_directory is not None:
            ...
            with open(
                f"{storing_directory}/mmd_train_vs_testset_values.pkl", "wb"
            ) as f:
