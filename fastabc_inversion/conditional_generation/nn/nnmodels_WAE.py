...
class netD(nn.Module):
    def __init__(self, nc=1, ndf=64, dfs=5, ngpu=1, ndata=10, npx=32, npy=32, dz=3):
        ...
        self.img_conv2d_1 = nn.Conv2d(
            self.nc, self.ndf, self.dfs, 2, self.dfs // 2, bias=False
        )
        self.img_conv2d_bn2d_1 = nn.BatchNorm2d(self.ndf)

        self.img_conv2d_2 = nn.Conv2d(
            self.ndf, self.ndf * 2, self.dfs, 2, self.dfs // 2, bias=False
        )
        self.img_conv2d_bn2d_2 = nn.BatchNorm2d(self.ndf * 2)

        self.img_conv2d_3 = nn.Conv2d(
            self.ndf * 2, self.ndf * 4, self.dfs, 2, self.dfs // 2, bias=False
        )
        self.img_conv2d_bn2d_3 = nn.BatchNorm2d(self.ndf * 4)

        self.img_conv2d_4 = nn.Conv2d(
            self.ndf * 4, 1, kernel_size=1, stride=1, padding=0, bias=False
        )
    ...
    def forward(self, input_images, input_labels):
        ...
        imgcode = F.leaky_relu(
            self.img_conv2d_bn2d_1(self.img_conv2d_1(input_images)), 0.2, inplace=True
        )
        imgcode = F.leaky_relu(
            self.img_conv2d_bn2d_2(self.img_conv2d_2(imgcode)), 0.2, inplace=True
        )
        imgcode = F.leaky_relu(
            self.img_conv2d_bn2d_3(self.img_conv2d_3(imgcode)), 0.2, inplace=True
        )
        imgcode = F.leaky_relu(
            self.img_conv2d_bn2d_4(self.img_conv2d_4(imgcode)), 0.2, inplace=True
        )
        imgcode = imgcode.view(-1, 16)

        measurecode = F.leaky_relu(
            self.measure_ln_bn_1(self.measure_ln_1(input_labels)), 0.2, inplace=True
        )
        measurecode = F.leaky_relu(
            self.measure_ln_bn_2(self.measure_ln_2(measurecode)), 0.2, inplace=True
        )
        measurecode = F.leaky_relu(
            self.measure_ln_bn_3(self.measure_ln_3(measurecode)), 0.2, inplace=True
        )

        fullcode = torch.cat((imgcode, measurecode), 1)
        fullcode = F.leaky_relu(
            self.code_ln_bn_1(self.code_ln_1(fullcode)), 0.2, inplace=True
        )
        fullcode = F.leaky_relu(
            self.code_ln_bn_2(self.code_ln_2(fullcode)), 0.2, inplace=True
        )
        fullcode = F.leaky_relu(
            self.code_ln_bn_3(self.code_ln_3(fullcode)), 0.2, inplace=True
        )
        fullcode = F.leaky_relu(
            self.code_ln_bn_4(self.code_ln_4(fullcode)), 0.2, inplace=True
        )
        fullcode = F.leaky_relu(
            self.code_ln_bn_5(self.code_ln_5(fullcode)), 0.2, inplace=True
        )
