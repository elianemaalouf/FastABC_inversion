...
To allow for kwargs in the limit state function
---------------------------------------------------------------------------
Original version 2018-05 
Adapted version 09-2021
---------------------------------------------------------------------------
Input:
...
def aCS(N, old_lam, b, u_j, G_LSF, **kwargs):
    ...
    for k in range(1, Ns + 1):
        ...
        geval[idx] = G_LSF(u_jp1[:, idx], **kwargs)  # store the lsf evaluation
        ...
        for t in range(1, Nchain[k - 1]):
            ...
            if Ge <= b:
                ...
                u_jp1[:, idx + t] = u_jp1[
                    :, idx + t - 1
                ]  # reject the candidate and use the same state
        ...
        if np.mod(k, Na) == 0:
            if Nchain[k - 1] > 1:
                ...
                lam[i + 1] = np.exp(
                    np.log(lam[i]) + zeta * (hat_a[i] - star_a)
                )  # Ref. 1 Eq. 26
    ...
    if i != 0:
        ...
        accrate = sum(acc[: np.mod(N, Ns)]) / np.mod(
            N, Ns
        )  # only for the few generated samples
