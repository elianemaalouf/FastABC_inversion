...
Script to run SuS inference on linear geophysics problem
"""

import time

import fastabc_inversion.geo_problems.Inference_Diagnostics as diags
from fastabc_inversion.geo_problems.Inference_Diagnostics import save_to_disk
from fastabc_inversion.geo_problems.jGNN_SuS_exp import \
    load_experiment_from_file

epoch = 1291
experiment_id = "id_8010k"
experiment_folder_name = "Lin_Geo_jGNN_SuS_Matern_81_id_8010k_5OV4I"
...
run_diagnostics = False  # whether to run diagnostics on the inference results
...
observation_idx_vec = [
    ...
    792,
    801,
]  # used in final inference observations

# vs training references - for validation experiments
...
rmse_x_refs_val = {
    "train": {"lower": 1.517, "center": 1.817, "upper": 2.186},
}
es_1_x_refs_val = {
    "train": {"lower": 1792.621, "center": 2016.382, "upper": 2278.220},
}
es_2_x_refs_val = {
    "train": {"lower": 2310.725, "center": 3301.291, "upper": 4535.569},
}
vs_05_x_refs_val = {
    "train": {"lower": 635041.558, "center": 707509.737, "upper": 796022.504},
}
...
refs_dict_x = refs_dict_x_val if len(observation_idx_vec) == 60 else refs_dict_x_inv
...
to_load = []  # 'rmse', 'scores', 'agg_diags', 'pf', 'post_samples
...
if run_diagnostics:
    ...
    if load_composite_samples:
        ...
        if composite_sample_metrics is None:
            composite_sample_metrics = inference_diags.compute_composite_sample_metrics(
...
if benchmark_inference:
    ...
    from fastabc_inversion.geo_problems.Inference_Diagnostics import (
        format_cca_metrics, format_cvae_metrics, load_from_disk)

    methods_metrics = {}

    # load exact solution metrics (analytical solution computed with true covariances not estimated covariances)
    analytical_solution_metrics = {"Exact": {}}
    analytical_solution_file = "FastABC/data/geo_linear/other_methods_inference_metrics/analytical/inversion_metrics_formatted.pkl"
    analytical_solution_metrics["Exact"] = load_from_disk(analytical_solution_file)
    methods_metrics.update(analytical_solution_metrics)

    # load CCA methods results and format them
    cca_metrics = {"CCA": {}}
    cca_metrics_file = "FastABC/data/geo_linear/other_methods_inference_metrics/CCA/inversion_metrics_formatted.pkl"
    cca_metrics["CCA"] = load_from_disk(cca_metrics_file)
    methods_metrics.update(cca_metrics)

    # load cVAE methods results and format them
    cvae_metrics_file = "FastABC/data/geo_linear/other_methods_inference_metrics/cVAE/inversion_metrics.pkl"
    cvae_metrics = load_from_disk(cvae_metrics_file)
    cvae_metrics_formatted = format_cvae_metrics(cvae_metrics, n_obs, noise_list)
    methods_metrics.update(cvae_metrics_formatted)
