...
It uses the same encoder-decoder parts as jGNN for X, but reduces the dependence of Z on Y.
"""
...
class netD(nn.Module):  # encoder
    def __init__(
        ...
        self.mix_code = nn.Sequential(
            # in: (b, 81 + ndata)
            nn.Linear(81 + ndata, self.dz),  # out: (b, dz)
            nn.BatchNorm1d(self.dz),  # out: ((b, dz)
            nn.PReLU(),  # out: (b, dz)
        )

        self.fc_mu = nn.Linear(self.dz, self.dz)
        self.fc_logvar = nn.Linear(self.dz, self.dz)

    def forward(self, x, y):
        # data = torch.cat([x, y], dim=1)  # concatenate image and measure
        # takes input: x, image (2000 : 50 x 40) and y, measure (81)
        batch_size = x.shape[0]

        img = x.view(batch_size, self.npx, self.npy).unsqueeze(1)

        measure = y.view(batch_size, self.ndata)
        ...
        if x.is_cuda and self.ngpu > 1:
            ...
            output = torch.cat((imgcode, measure), 1)
            output = nn.parallel.data_parallel(self.mix_code, output, range(self.ngpu))
