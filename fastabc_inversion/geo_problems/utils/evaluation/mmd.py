...
def MMD2(X, Y, kernel_params, unbiased=True):
    ...
    if kernel_params.get("metric", "rbf") == "rbf":
        if "gamma" not in kernel_params:
            print(
                "kernel: RBF - estimating gamma using median heuristic on provided sample."
            )
            sigma2 = estimate_median_pairwise_dists(
                XY, sample_ratio=1.0
            )  # does not count 0 distances in median
            # sigma2 = np.median(pairwise_distances(XY, metric='euclidean')) ** 2 # counts also 0 distances in median (original version)
